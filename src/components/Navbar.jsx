...
//         {
//           currentUser ? (
//             <Avatar onClick={handleProfileModal} round={true} className=' object-cover' name={currentUser?.channelName} src={currentUser?.profilePicture} size="32" textSizeRatio={1.75} />
//           ) : (
//             <>
...
const Navbar = () => {
  ...
  // Close profile modal and navigate to profile page
        ...
        {isProfileModalOpen && (
          <div className="absolute top-12 right-0 bg-white w-64 h-72 border border-gray-300 rounded-lg shadow-lg z-500">
            <div className="flex flex-col items-center justify-between h-full p-4">
              ...
              <div className="mt-4 w-full flex flex-col space-y-2">
                <button
                  onClick={() => handleProfileNavigation(currentUser._id)} // Close modal and navigate
                  className="w-full py-2 px-4 text-sm font-medium text-blue-600 hover:bg-blue-100 rounded-md border border-blue-600 transition-all"
                >
                  Your Channel
                </button>
                <button
